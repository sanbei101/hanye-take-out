          ...
          <div v-if="form.dishFlavors.length != 0" class="flavor">
            ...
            <div class="cont">
              <div v-for="(item, index) in form.dishFlavors" :key="index" class="items">
                <div class="itTit">
                  <SelectInput :dish-flavors-data="leftDishFlavors" :index="index" :value="item.name" @select="selectHandle" />
                </div>
                <div class="labItems" style="display: flex">
                  <span v-for="(it, ind) in item.list" :key="ind">{{ it }} <i @click="delFlavorLabel(index, ind)">X</i></span>
                  <div class="inputBox"></div>
                </div>
                <span class="delFlavor delBut non" @click="delFlavor(item.name)">删除</span>
              </div>
            </div>
            <div v-if="!!leftDishFlavors.length && form.dishFlavors.length < dishFlavorsData.length" class="addBut" @click="addFlavor">
              添加口味
            </div>
          </div>
        </div>
