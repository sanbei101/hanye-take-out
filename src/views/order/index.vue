...
<template>
  <div class="dashboard-container">
    <el-tabs v-model="activeIndex">
      <el-tab-pane v-for="item in changedOrderList" :key="item.value" :label="item.label" :name="item.value.toString()">
        <el-badge :value="item.num && item.num > 99 ? '99+' : item.num" :hidden="![2, 3, 4].includes(item.value)"> </el-badge>
      </el-tab-pane>
    </el-tabs>
  </div>
  <div class="container">
    ...
    <el-table v-if="tableData.length" :data="tableData" stripe class="tableBox">
      ...
      <el-table-column v-if="[0, 5, 6].includes(orderStatus)" key="consignee" prop="consignee" label="用户名" show-overflow-tooltip />
      ...
      <el-table-column
        ...
        <template #default="scope">
          <div class="btn_box">
            <div class="before">
              <el-button v-if="scope.row.status === 2" type="primary" link @click="orderAccept(scope.row), (isTableOperateBtn = true)">
                接单
              </el-button>
              <el-button v-if="scope.row.status === 3" type="primary" link @click="deliveryOrComplete(3, scope.row.id)"> 派送 </el-button>
              <el-button v-if="scope.row.status === 4" type="primary" link @click="deliveryOrComplete(4, scope.row.id)"> 完成 </el-button>
            </div>
            <el-divider direction="vertical" />
            <div class="middle">
              <el-button v-if="scope.row.status === 2" type="danger" link @click="orderReject(scope.row), (isTableOperateBtn = true)">
                拒单
              </el-button>
              <el-button v-if="[1, 3, 4, 5].includes(scope.row.status)" type="danger" link @click="cancelOrder(scope.row)">
                取消
              </el-button>
            </div>
            <el-divider direction="vertical" />
            <div class="after">
              <el-button type="primary" link class="blueBtn" @click="goDetail(scope.row.id, scope.row.status, scope.row)"> 查看 </el-button>
            </div>
          </div>
        </template>
      </el-table-column>
  ...
  <el-dialog title="订单信 息" v-model="dialogVisible" width="53%" :before-close="handleClose" class="order-dialog">
    <el-scrollbar style="height: 100%">
      <div class="order-top">
        <div>
          ...
          <div style="display: inline-block" class="order-status" :class="{ status3: [3, 4].includes(dialogOrderStatus) }">
      ...
      <div class="order-middle">
        ...
        <div class="dish-info">
          ...
          <div class="dish-all-amount">
            <label>菜品小计</label>
            <span
              >￥{{ diaForm && diaForm.amount && diaForm?.packAmount ? (diaForm!.amount - 6 - diaForm!.packAmount).toFixed(2) : 0 }}</span
            >
          </div>
        </div>
      </div>
    ...
    <template #footer>
      <span v-if="dialogOrderStatus !== 6" class="dialog-footer">
        ...
        <div>
          <el-button v-if="dialogOrderStatus === 2" @click="orderReject(my_row), (isTableOperateBtn = false)">拒 单</el-button>
          <el-button v-if="dialogOrderStatus === 2" type="primary" @click="orderAccept(my_row), (isTableOperateBtn = false)"
            >接 单</el-button
          >

          <el-button v-if="[1, 3, 4, 5].includes(dialogOrderStatus)" @click="dialogVisible = false">返 回</el-button>
          <el-button v-if="dialogOrderStatus === 3" type="primary" @click="deliveryOrComplete(3, my_row!.id)">派 送</el-button>
          <el-button v-if="dialogOrderStatus === 4" type="primary" @click="deliveryOrComplete(4, my_row!.id)">完 成</el-button>
