...
const rules = {
  ...
  setmealDishes: [{ required: true, trigger: 'blur', message: '不能为空' }],
...
<template>
  ...
  <el-card>
    <el-dialog v-if="dialogVisible" title="添加菜品" class="addDishList" v-model="dialogVisible" width="60%" :before-close="handleClose">
      ...
      <AddDish v-if="dialogVisible" ref="adddish" :check-list="selectList" :search-key="searchKey" @selectList="getSelectList" />
            ...
            <div class="table">
              <el-table :data="dishTable" style="width: 100%">
                ...
                <el-table-column prop="operation" label="操作" width="180px;">
                  <template v-slot="scope">
                    <el-button link type="danger" size="small" class="delBut non" @click="delDishHandle(scope.$index)"> 删除 </el-button>
                  </template>
                </el-table-column>
              </el-table>
            </div>
